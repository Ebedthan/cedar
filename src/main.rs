...
use crate::{
    bootstrap::sample_sketches_with_replacement,
    cli::Commands,
    dist::{ComputeTree, TreeAlgorithm},
    nwk::Tree,
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Build(args) => {
            let filenames = args.input.as_slice();

            // Validate inputs
            if let Err(e) = utils::validate_inputs(&args.input) {
            ...
            if let Some(km) = args.kmer {
                ...
                let mean_genome_size =
                    stats.iter().map(|x| x.1 as u32).sum::<u32>() / stats.len() as u32;
            ...
            let mut reps = 100;
            if let Some(r) = args.bootstrap {
                println!("Boostraping replicates: {}", r);
                reps = r;
            }

            // Create temporary directory
            fs::create_dir_all(&args.tempdir).context(format!(
            ...
            let tree_algorithm: TreeAlgorithm =
                TreeAlgorithm::from_cli(args.canonical, cli.threads);
            ...
            if args.bootstrap.is_some() {
                ...
                for _ in 0..reps {
                    ...
                    let tmp_tree = tmp_matrix.compute_newick_tree(&tree_algorithm)?;
                ...
                utils::output_tree(args.output, consensus_tree.to_newick())?;
            } else {
